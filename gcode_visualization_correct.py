...
img = plt.imread(os.path.join('.', 'cropped', 'corrected.png'))
...
plt.scatter(X_cor, Y_cor, c='#FFA500')
# print(X_obs[0])
# print(Y_obs[0])
...
plt.savefig('cropped/corrected_perfect_and_points.png', transparent=False)
...
plt.savefig('cropped/corrected_points.png', transparent=False)
...
for i in range(len(X_cor)):
    ...
    X[fst, snd] = X_cor[i]
    Y[fst, snd] = Y_cor[i]
    U[fst, snd] = X_obs[i] - X_cor[i]
    V[fst, snd] = Y_obs[i] - Y_cor[i]
    plt.arrow(X[fst, snd], Y[fst, snd], U[fst, snd], V[fst, snd])
### Quiver
# plt.quiver([X, Y], U, V)

plt.savefig('cropped/corrected_quiver.png', transparent=False)
